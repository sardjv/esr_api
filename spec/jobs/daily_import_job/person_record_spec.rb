describe DailyImportJob, type: :job do
  let(:add_filename) { file_fixture('add_person_record.DAT').to_path }
  let(:update_filename) { file_fixture('update_person_record.DAT').to_path }
  let(:delete_filename) { file_fixture('delete_person_record.DAT').to_path }

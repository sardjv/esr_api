describe DailyImportJob, type: :job do
  let(:add_filename) { file_fixture('add_person_eit_record.dsv').to_path }
  let(:update_filename) { file_fixture('update_person_eit_record.dsv').to_path }
  let(:delete_filename) { file_fixture('delete_person_eit_record.dsv').to_path }
  ...
  it 'creates a new PersonEitRecord' do
    ...
    # Expect values in the database to match input from add_person_eit_record.dsv.
  ...
  context 'with an existing PersonEitRecord' do
    ...
    it 'updates it' do
      ...
      # Expect values in the database to match input from update_person_eit_record.dsv.

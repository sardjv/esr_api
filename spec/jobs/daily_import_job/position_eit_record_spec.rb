describe DailyImportJob, type: :job do
  let(:add_filename) { file_fixture('add_position_eit_record.dsv').to_path }
  let(:update_filename) { file_fixture('update_position_eit_record.dsv').to_path }
  let(:delete_filename) { file_fixture('delete_position_eit_record.dsv').to_path }

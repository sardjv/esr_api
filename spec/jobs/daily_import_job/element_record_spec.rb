describe ImportFileJob, type: :job do
  let(:add_filename) { file_fixture('add_element_record.DAT').to_path }
  let(:update_filename) { file_fixture('update_element_record.DAT').to_path }
  let(:delete_filename) { file_fixture('delete_element_record.DAT').to_path }

describe DailyImportJob, type: :job do
  let(:add_filename) { file_fixture('add_location_record.dsv').to_path }
  let(:update_filename) { file_fixture('update_location_record.dsv').to_path }
  let(:delete_filename) { file_fixture('delete_location_record.dsv').to_path }
  ...
  it 'creates a new LocationRecord' do
    ...
    # Expect values in the database to match input from add_location_record.dsv.
    ImportExpectations.location_record.each do |key, value|
  ...
  context 'with an existing LocationRecord' do
    ...
    it 'updates it' do
      ...
      # Expect values in the database to match input from update_location_record.dsv.

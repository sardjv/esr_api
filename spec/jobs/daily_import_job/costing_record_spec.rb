describe DailyImportJob, type: :job do
  let(:add_filename) { file_fixture('add_costing_record.DAT').to_path }
  let(:update_filename) { file_fixture('update_costing_record.DAT').to_path }
  let(:delete_filename) { file_fixture('delete_costing_record.DAT').to_path }

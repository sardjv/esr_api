describe ImportFileJob, type: :job do
  let(:add_filename) { file_fixture('good_imports/add_disability_record.DAT').to_path }
  let(:update_filename) { file_fixture('good_imports/update_disability_record.DAT').to_path }
  let(:delete_filename) { file_fixture('good_imports/delete_disability_record.DAT').to_path }
  ...
  it 'creates a new DisabilityRecord' do
    ...
    # Expect values in the database to match input from add_disability_record.DAT.
  ...
  context 'with an existing DisabilityRecord' do
    ...
    it 'updates it' do
      ...
      # Expect values in the database to match input from update_disability_record.DAT.

describe ImportFromFtpJob, type: :job do
  ...
  describe 'add' do
    let(:path) { 'good_imports/sit_record/add' }
  ...
  describe 'update' do
    let(:path) { 'good_imports/sit_record/update' }
    ...
    context 'with an existing SitRecord' do
      before do
        build(:sit_record) do |r|
          r.assign_attributes(
            Expectations::SitRecord.added.merge(
      ...
      it 'versions it' do
        ...
        expect(SitRecord.first.versions.last.whodunnit_type).to eq('Import')
        expect(SitRecord.first.versions.last.whodunnit).to include('update')
      end
    end
  end

  describe 'delete' do
    let(:path) { 'good_imports/sit_record/delete' }

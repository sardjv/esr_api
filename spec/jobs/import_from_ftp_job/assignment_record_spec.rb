describe ImportFromFtpJob, type: :job do
  ...
  describe 'add' do
    let(:path) { 'good_imports/assignment_record/add' }
  ...
  describe 'update' do
    let(:path) { 'good_imports/assignment_record/update' }
    ...
    context 'with an existing AssignmentRecord' do
      before do
        build(:assignment_record) do |r|
          r.assign_attributes(
            Expectations::AssignmentRecord.added.merge(
      ...
      it 'versions it' do
        ...
        expect(AssignmentRecord.first.versions.last.whodunnit_type).to eq('Import')
        expect(AssignmentRecord.first.versions.last.whodunnit).to include('update')
      end
    end
  end

  describe 'delete' do
    let(:path) { 'good_imports/assignment_record/delete' }

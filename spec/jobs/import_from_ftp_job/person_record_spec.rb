describe ImportFromFtpJob, type: :job do
  ...
  describe 'add' do
    let(:path) { 'good_imports/person_record/add' }
  ...
  describe 'update' do
    let(:path) { 'good_imports/person_record/update' }
    ...
    context 'with an existing PersonRecord' do
      before do
        build(:person_record) do |r|
          r.assign_attributes(
            Expectations::PersonRecord.added.merge(
      ...
      it 'versions it' do
        ...
        expect(PersonRecord.first.versions.last.whodunnit_type).to eq('Import')
        expect(PersonRecord.first.versions.last.whodunnit).to include('update')
      end
    end
  end

  describe 'delete' do
    let(:path) { 'good_imports/person_record/delete' }

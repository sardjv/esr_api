...
describe 'Api::V1::CompetenceDefinitionRecordResource', type: :request, swagger_doc: 'v1/swagger.json' do
  let!(:competence_definition_record) { create(:competence_definition_record) }
  let!(:competence_definition_record2) { create(:competence_definition_record) }
  let(:response_data) { JSON.parse(response.body)['data'] }

  path '/api/v1/competence_definition_records' do
    get 'competence definition records' do
      tags 'CompetenceDefinitionRecord'
      security [{ Token: {} }]
      ...
      context 'with a token and at least 1 confirmed user on the system' do
        ...
        let(:columns) { ETL::Headers::CompetenceDefinitionRecord.api_headers }
        ...
        context 'with a permission with the wrong action' do
          let(:resource) { 'CompetenceDefinitionRecord' }
          let(:action) { 'show' }
        ...
        context 'with a permission with the right resource and action' do
          ...
          context 'with a subset of columns' do
            let(:columns) { ETL::Headers::CompetenceDefinitionRecord.api_headers[0..4] }

            response '200', 'successful' do
              schema '$ref' => '#/definitions/competence_definition_records_response'

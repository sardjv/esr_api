...
class OrganisationRecordDashboard < Administrate::BaseDashboard
  ...
  ATTRIBUTE_TYPES = {
    ...
    'Effective From Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                           field.data&.strftime('%d/%m/%Y')
                                                                         }),
    'Effective To Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                         field.data&.strftime('%d/%m/%Y')
                                                                       }),
    'Hierarchy Version ID': Field::String,
    'Hierarchy Version Date From': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                                   field.data&.strftime('%d/%m/%Y')
                                                                                 }),
    'Hierarchy Version Date To': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                                 field.data&.strftime('%d/%m/%Y')
                                                                               }),

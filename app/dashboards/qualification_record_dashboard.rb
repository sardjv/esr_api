...
class QualificationRecordDashboard < Administrate::BaseDashboard
  ...
  ATTRIBUTE_TYPES = {
    ...
    'Status': Field::String,
    'Grade': Field::String,
    'Awarded Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                    field.data&.strftime('%d/%m/%Y')
                                                                  }),
    'Start Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                  field.data&.strftime('%d/%m/%Y')
                                                                }),
    'End Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                field.data&.strftime('%d/%m/%Y')
                                                              }),

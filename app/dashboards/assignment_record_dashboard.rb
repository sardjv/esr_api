...
class AssignmentRecordDashboard < Administrate::BaseDashboard
  ...
  ATTRIBUTE_TYPES = {
    ...
    'Earliest Assignment Start Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                                      field.data&.strftime('%d/%m/%Y')
                                                                                    }),
    ...
    'Start Date in Grade': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                           field.data&.strftime('%d/%m/%Y')
                                                                         }),
    ...
    'Projected Hire Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                           field.data&.strftime('%d/%m/%Y')
                                                                         }),
    'Vacancy ID': Field::String,
    'Contract End Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                         field.data&.strftime('%d/%m/%Y')
                                                                       }),
    'Increment Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                      field.data&.strftime('%d/%m/%Y')
                                                                    }),
    'Maximum Part Time Flag': Field::String,
    'AFC Flag': Field::String,
    'Last Update Date': Field::DateTime,
    'Last Working Day': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                        field.data&.strftime('%d/%m/%Y')
                                                                      }),
    'e-KSF Spinal Point': Field::String,
    'Manager Flag': Field::String,
    'Assignment End Date': Field::Date.with_options(transform_on_export: lambda { |field|
                                                                           field.data&.strftime('%d/%m/%Y')
                                                                         }),

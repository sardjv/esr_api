...
The project is a work in progress but when complete should make it easier for NHS trusts to build useful applications on top of centralised ESR data. The project is released as open source code so that other NHS trusts can use the software as needed.

## Use Cases

- Improve efficiency of functions such as Business Intelligence.
- Connect other systems such as Job Planning with the latest data on staff.
- Reduce time spent generating reports.
- Find out training status of staff.
- Quickly build new app frontends to address the needs of your trust.
